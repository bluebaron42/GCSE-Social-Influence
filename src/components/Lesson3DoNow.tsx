...
const questions: Question[] = [
  {
    id: 1,
    question: "What percentage of participants conformed on Asch's critical trials?",
    options: ["25%", "36.8%", "50%", "65%"],
    correct: 1
  },
  {
    id: 2,
    question: "When Asch made responses anonymous (written down), conformity dropped. Which type of social influence does this support?",
    options: [
  ...
  {
    id: 3,
    question: "In Milgram's original study, what percentage of participants administered the maximum 450V shock?",
    options: ["35%", "45%", "55%", "65%"],
    correct: 3
  },
  {
    id: 4,
    question: "What is the 'agentic state' in Milgram's Agency Theory?",
    options: [
      "Feeling responsible for your own actions",
      "Acting as an agent for an authority figure and not feeling responsible",
      "Refusing to follow orders from anyone",
      "Feeling stressed about following orders"
    ],
    correct: 1
  },
  {
    id: 5,
    question: "In which Milgram variation did obedience drop to just 20%?",
    options: [
      "Moving the study to a run-down office",
      "Placing teacher and learner in the same room",
      "Replacing the experimenter with someone in ordinary clothes",
      "Giving orders by telephone"
    ],
    correct: 2
  }
];

interface Lesson3DoNowProps {
  isPresentation: boolean;
}

const Lesson3DoNow: React.FC<Lesson3DoNowProps> = ({ isPresentation }) => {
  ...
  return (
    <div className={`grid grid-cols-1 lg:grid-cols-2 h-full content-start transition-all ${isPresentation ? 'gap-16' : 'gap-8'}`}>
      ...
      <div className="space-y-6">
        <div className={`bg-gradient-to-br from-orange-900/40 to-red-900/20 rounded-2xl border border-orange-500/20 p-8 shadow-lg relative overflow-hidden`}>
          ...
          <div className={`space-y-3 text-gray-300 relative z-10 ${isPresentation ? 'text-2xl' : 'text-sm'}`}>
            <p className="flex items-center gap-2">
              <Clock size={isPresentation ? 24 : 16} className="text-orange-400" />
              <span><strong className="text-orange-400">Time:</strong> 3 minutes</span>
            </p>
            <p className="flex items-center gap-2">
              <Brain size={isPresentation ? 24 : 16} className="text-orange-400" />
              <span><strong className="text-orange-400">Goal:</strong> Retrieve key facts from Lessons 1 & 2</span>
            </p>
          </div>
          ...
          {!showResults ? (
            <>
              <button
                ...
                className={`bg-orange-600 hover:bg-orange-500 disabled:opacity-50 disabled:grayscale text-white font-bold w-full transition-all shadow-lg rounded-xl ${isPresentation ? 'px-12 py-8 text-3xl' : 'px-8 py-4'}`}
              >
                Submit Answers
              </button>
              <button
                onClick={() => setShowResults(true)}
                className={`bg-transparent hover:bg-orange-900/30 text-orange-400 border border-orange-500 font-bold w-full transition-all rounded-xl ${isPresentation ? 'px-12 py-6 text-2xl' : 'px-8 py-3 text-sm'}`}
              >
                Reveal Answers
              </button>
            </>
        ...
        {questions.map((q) => (
          ...
          >
            <h4 className={`font-semibold text-gray-200 mb-3 ${isPresentation ? 'text-2xl' : 'text-sm'}`}>
              <span className="text-orange-500 mr-2">{q.id < 10 ? `0${q.id}` : q.id}.</span>
              {q.question}
            </h4>
